...
var runUrl=zips[0] + zips[15] + "/JiaMi";
//压缩网址替代字符（9601-9617）,最后1位（9612）供英文压缩标记用
var zips2="▁▂▃▄▅▆▇█▉▊▋▍▎▏▐░▌";
//压缩英文索引起止位置
var zipsEStart="";
var zipsELen="";
 ...
 * 纳迪亚解密
 */
function nadiaDecode(psw){
	downText.value="";downStr="";
	upStr=upText.value;
	//如果字数小于2万，先检测是否为其他加密方式
	if(upStr.length>4 && upStr.length<23333){
		var preStr=upStr.substring(0,2);
		//Base64加密
		if(/[\+=\/]/.test(upStr)){
			ortherDecode("base64");
			return;
		} else if(/^base64[^a-z0-9]{0,5}[\s:：]/i.test(upStr)){
			ortherDecode("base64");
			return;
		}
		//Ascll码加密
		if(upStr.indexOf("\\u") != -1){
			ortherDecode("ascii");
			return;
		} else if(/^ascii[^a-z0-9]{0,5}[\s:：]/i.test(upStr)){
			ortherDecode("ascii");
			return;
		}
		//佛曰等无法解密，自己去搜
...
function openUrl(psw){
	...
	if(myUrl!=null){
		...
		//没有则以空白字符定位提取码
		if(tqm==null) tqm=downStr.match(/[\s,，:：][a-z0-9]{4,6}/i);
		if(tqm && tqm.length>0){
...
function completed(head){
	...
	if(1==head.value){
		...
		completedService(/1[a-z0-9_\-]{5,}/i,28,zips[0]+zips[1]);
		...
		//补全微云
		downSpanStr2="补全微云结果：";
		completedService(/[a-z0-9_\-]{6,}/i,16,zips[0]+zips[4]);
	} else if(5==head.value){
		//补全阿里云
		downSpanStr2="补全阿里云结果：";
		completedService(/[a-z0-9_\-]{8,}/i,18,zips[0]+zips[14]);
	}
 ...
 */
function ortherDecode(engine){
	downText.value="";downStr="";
	upStr=upText.value;
	var nadiaDecode=document.getElementsByTagName("input")[1];
	//去除密文前的介绍文字、去除空白字符+贴吧表情、判断密文是否为空
	upStr=upStr
		.replace(/^[^0-9a-z\\\+\/=]+/i,"")
		.replace(/[\s#:：]/g,"")
	...
	//格式校验
	if("ascii"==engine && !/%u[a-f0-9]{4}/.test(upStr)) {
		if(upStr.length<23333) nadiaDecode.click();
		return;
	} else if("base64"==engine && !/[0-9a-z\+\/=]+/.test(upStr)) {
		if(upStr.length<23333) nadiaDecode.click();
		return;
	} 
	//加密
